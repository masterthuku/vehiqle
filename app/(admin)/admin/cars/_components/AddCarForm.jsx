...
const AddCarForm = () => {
  ...
  if (processImageResult?.success) {
    ...
    if (uploadedAiImage && uploadedAiImage instanceof Blob) {
      const reader = new FileReader();
      reader.onload = (e) => {
        setUploadedImages((prev) => [...prev, e.target.result]);
      };
      reader.onerror = () => toast.error("Failed to process AI image");
      reader.readAsDataURL(uploadedAiImage);
    } else if (typeof uploadedAiImage === "string" && uploadedAiImage.startsWith("data:")) {
      // Fallback if it's already a data URL
      setUploadedImages((prev) => [...prev, uploadedAiImage]);
    } else {
      console.error("Invalid uploadedAiImage:", uploadedAiImage);
      toast.error("Image processing failed - invalid file format");
    }

    toast.success("Car details extracted successfully!", {
      description: `Detected ${carDetails.year} ${carDetails.make} ${carDetails.model} with ${Math.round((carDetails.confidence || 0) * 100)}% confidence`,
    });
